...
function PlayerManager:SetSettleAccounts(playerInfoList,winChairdId)
	if playerInfoList and #playerInfoList>0 then
		...
		for i=1,#playerInfoList do
			...
			if playerIns then
				...
				if winChairdId==playerInfoList[i].chairId then
...
function PlayerManager:SetPlayerGoldFlyEffect(playerAccountStatelist)
	local GoldFlyEffectFunc=function ()
		...
		if #playerAccountStatelist.LoseInsList>0 then
			...
			yield_return(WaitForSeconds(0.8))
			HeadEffect=ParticleManager.GetInstance():GetAllocateEffect("PlayerHeadEffect")
			HeadEffect.transform.position=zhuangJiaEffectPos
		end
		

		yield_return(WaitForSeconds(2))
		if HeadEffect then
			ParticleManager.GetInstance():RecycleEffect(HeadEffect)
		end
