...
function BulletManager:LocalCreatBullet(myChairId,bulletUID,bulletSpeedIndex,bulletIntervalTimeIndex,bulletLevel)
	local msg={}
	...
	msg.BulletLevel=PlayerManager.GetInstance():GetRealGunLevel(bulletLevel)
...
function BulletManager:ParseBulletMsg(msg)
	...
	vo.BulletLevel=PlayerManager.GetInstance():GetRealGunLevel(msg.usLevelIndex)--msg.usLevelIndex
...
function BulletManager:GetBullet(vo)
	local bulletLevel=vo.BulletLevel or 1
	if self.AllBulletInsList and self.AllBulletInsList[bulletLevel] and next(self.AllBulletInsList[bulletLevel]) then
		local bulletIns=table.remove(self.AllBulletInsList[bulletLevel],1)
		if bulletIns then
		...
		local tempBT=GameObjectPoolManager.GetInstance():GetGameObject(self.BulletPrefabName..bulletLevel,GameObjectPoolManager.PoolType.BulletPool)
		if tempBT==nil then
			Debug.LogError("子弹创建失败==>"..bulletLevel)
			--GameManager.GetInstance():ShowUITips("子弹创建失败==>"..bulletLevel,2)
			return
		end
...
function BulletManager:RecycleBullet(bulletIns)
	...
	if tempBullet then
		if not self.AllBulletInsList[bulletIns.BulletVo.BulletLevel] then
			self.AllBulletInsList[bulletIns.BulletVo.BulletLevel]={}
		end
		table.insert(self.AllBulletInsList[bulletIns.BulletVo.BulletLevel],tempBullet)
