...
function FishManager:AddScripts()
	self.ScriptsPathList={
		...
		"/View/Fish/FishVo/PartFish",
		"/View/Fish/FishVo/DragonFish",
		}
end
...
function FishManager:InitInstance()
	...
	self.PartFish = PartFish
	self.DragonFish = DragonFish
end
...
function FishManager:GetFish(vo,isAddCurrentUseFishList)
	if self.AllUsedFishInsList and self.AllUsedFishInsList[vo.FishId] and next(self.AllUsedFishInsList[vo.FishId]) then
		...
		if FishItem then
			...
			elseif rawFishType== self.gameData.GameConfig.FishType.Part then
				fish=self.PartFish.New()
			elseif rawFishType== self.gameData.GameConfig.FishType.Dragon then
				fish=self.DragonFish.New()
			else
...
function FishManager:ClearOtherPlayerKillFish(chairID)
	if self.CurrentUseFishList  then
		for k,v in pairs(self.CurrentUseFishList) do
			if (v:GetIsDie() == true) and (v.chairdID == chairID)  and (v.FishVo.DieEffectConfig.fishDieBehavior == 3) then
				v:Destroy()
				self:RemoveFish(v)	
			end
		end
...
function FishManager:GetCacheFishByID(FishUID)
...
function FishManager:GetFishSortingOrderIndex(fish)
	...
	if self.FishSortingOrderList[fish.FishVo.FishId]<fish.FishVo.FishConfig.layerMax then
		self.FishSortingOrderList[fish.FishVo.FishId]=self.FishSortingOrderList[fish.FishVo.FishId]+2
...
function FishManager:AddEventListenner()
	...
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEDIANCICANNON_RSP"),self.ResponesCreateDianCiCannonMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEZUANTOU_RSP"),self.ResponesCreateZuanTouMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATESOMEZUANTOU_RSP"),self.ResponesCreateSerialZuanTouMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEFIRESTORM_RSP"),self.ResponesCreateFireStormMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEMADCOW_RSP"),self.ResponesCreateBisonMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEDELAYBOMB_RSP"),self.ResponesCreateBombMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATESERIALBOMBCRAB_RSP"),self.ResponesCreateMultBombMsg,self)

	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_HAIWANGCRAB_KILLEDPART_RSP"),self.ResponesKillFishPartMsg,self)
	LuaNetwork.RegisterNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_HAIWANGCRABKILLEDDEAD_RSP"),self.ResponesKillFishDeadMsg,self)
...
function FishManager:RemoveEventListenner()
	...
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEDIANCICANNON_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEZUANTOU_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATESOMEZUANTOU_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEFIRESTORM_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEMADCOW_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATEDELAYBOMB_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_CREATESERIALBOMBCRAB_RSP"))

	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_HAIWANGCRAB_KILLEDPART_RSP"))
	LuaNetwork.RemoveNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_HAIWANGCRABKILLEDDEAD_RSP"))
end
...
function FishManager:RequestPlayerHitFishPartMsg(sendMsg)
	local bytes = LuaProtoBufManager.Encode("Fish_Msg.HaiWangCrabHitPartReq",sendMsg)
	GameManager.GetInstance():SendNetMessage(LuaProtoBufManager.Enum("Fish_Msg.Proto_Fish_CMD","NF_FISH_CMD_HAIWANGCRABHITPART_REQ"),bytes)
end
...
function FishManager:ResponesCreateDianCiCannonMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateDianCiCannonRsp",msg)
	ElectricSkillManager.GetInstance():EnterEletricSkillMode(data)
end

function FishManager:ResponesCreateZuanTouMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateZuanTouRsp",msg)
	
	DrillSkillManager.GetInstance():EnterDrillSkillMode(data)
end

function FishManager:ResponesCreateSerialZuanTouMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateSomeZuanTouRsp",msg)
	
	SerialDrillSkillManager.GetInstance():EnterSerialDrillSkillMode(data)
end

function FishManager:ResponesCreateFireStormMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateFireStormRsp",msg)
	
	FireStormSkillManager.GetInstance():EnterFireStormSkillMode(data)
end

function FishManager:ResponesCreateBisonMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateMadCowRsp",msg)
	
	BisonSkillManager.GetInstance():EnterBisonSkillMode(data)
end

function FishManager:ResponesCreateBombMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateDelayBombRsp",msg)
	
	BombSkillManager.GetInstance():EnterBombSkillMode(data)
end

function FishManager:ResponesCreateMultBombMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.CreateSerialBombCrabRsp",msg)
	MultBombSkillManager.GetInstance():EnterMultBombSkillMode(data)
end

function FishManager:ResponesKillFishPartMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.HaiWangCrabKilledPartRsp",msg)
	BombManager.GetInstance():KillPartFishSection(data)
end

function FishManager:ResponesKillFishDeadMsg(msg)
	local data=LuaProtoBufManager.Decode("Fish_Msg.HaiWangCrabKilledDeadRsp",msg)
	local playerIns=PlayerManager.GetInstance():GetPlayerInsByChairdId(data.usChairId)
	local hitFish=FishManager.GetInstance():GetUsingFishByFishUID(data.usHaiwangCrabId)
	SpiderCrabEffectManager.GetInstance():CreateSpiderCrabBoardScore(data.usChairId,data.usPartMul,data.usSelfScore,data.usTotalScore,data.usTotalMul,playerIns.SpecialDeclarePanel.position)
end
