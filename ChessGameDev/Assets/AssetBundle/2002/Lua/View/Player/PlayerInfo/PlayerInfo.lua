...
function PlayerInfo:InitData()
	...
	self.ShootBulletRate={0.4,0.2,0.1}		--射击间隔
...
function PlayerInfo:OnclickAddBet()
	...
	pt(self.gameData.GunLevelConfig[tempGunLevel])
...
function PlayerInfo:SendShootBullet()
	...
	self:LocalCreateBullet(shootAngle)
end
...
function PlayerInfo:LocalCreateBullet(shootAngle)
	if self.ChairdID==self.gameData.PlayerChairId then
		BulletManager.GetInstance():LocalCreatBullet(self.ChairdID,self.BulletUID,1,1,1,shootAngle)
