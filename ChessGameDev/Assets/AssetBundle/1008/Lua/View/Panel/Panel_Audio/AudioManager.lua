...
function AudioManager:FindView()
	self.AudioSource=GameManager.GetInstance().AudioSource
	self.AudioClip=GameManager.GetInstance().AudioClip
...
function AudioManager:PlayNormalAudio(audioIndex,volume,isLoop)
	...
	local audioInfo=self:GetAudioInfo(audioIndex)
	if audioInfo==nil then Debug.LogError("获取音频失败===>>>"..audioIndex)  return end
...
function AudioManager:IsEnableBGAudio(isEnable)
	self.AudioBG.enabled=isEnable
end
