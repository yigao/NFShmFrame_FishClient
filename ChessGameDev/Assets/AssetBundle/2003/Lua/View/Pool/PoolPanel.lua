...
function PoolPanel:FindView(tf)
	...
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_PlusTips").gameObject	--6：Pool_PlusTips
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_Effect").gameObject	--7：Pool_Effect
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_FishOutTips").gameObject	--8：FishOutTips
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_BigAwardTips").gameObject	--9：BigAwardTips
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_Lightning").gameObject	--10：Lightning
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_Other").gameObject	--11：Other
	table.insert(self.PoolList,PoolObj)
	PoolObj=tf:Find("Pool_TipsContent").gameObject	--12：Pool_TipsContent
	table.insert(self.PoolList,PoolObj)
end
