...
function GameManager:InitGameConfigData(data)
	...
	if data.cols then
		...
		if data.gameType==StateManager.MainState.BBG or data.gameType==StateManager.MainState.SelectMiniGame then
			StateManager.GetInstance():SetNextMainStation(data.gameType)
			GameUIManager.GetInstance():SetBBGInitData()
		end
		
		for i=1,5 do
			local iconRandomValue=math.random(0,8)
...
function GameManager:InitGameState(gameType)
	...
	if gameType>0 then
		...
		elseif gameType==StateManager.MainState.SelectMiniGame then
			GameLogicManager.GetInstance():IsSelectGameProcess()
		elseif gameType==StateManager.MainState.BBG then
