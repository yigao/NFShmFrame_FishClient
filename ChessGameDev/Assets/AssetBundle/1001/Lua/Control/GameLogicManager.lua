...
function GameLogicManager:ResetIconRunData()
	...
	BaseFctManager.GetInstance():SetWinTipsText(1)
...
function GameLogicManager:SetStart()
	...
	AudioManager.GetInstance():PlayNormalAudio(5)
	...
	local StopGameIconRunProcessFunc=function ()
		...
		for i=1,self.GameConfig.Coordinate_COL do
			...
			if tempColumnIconResult then
				if CommonHelper.IsContainValueForDic(9,tempColumnIconResult) then
...
local freeIconAudioIndex={21,22,23}
--每列停止后回调
...
function GameLogicManager:ShowSpecialIconAnim(columnIndex,tempIconResult)
	for i=1,#tempIconResult do
		if tempIconResult[i]==9 or tempIconResult[i]==8 then
...
function GameLogicManager:SetWinLineTips()
	BaseFctManager.GetInstance():SetWinTipsText(2,self.gameData.TotalPrizeLine)
end
