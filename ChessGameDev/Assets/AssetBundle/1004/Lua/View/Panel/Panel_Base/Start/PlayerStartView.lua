...
function PlayerStartView:FindPlayerStartView(tf)
	...
	self.SpinePanel=tf:Find("Down/Spine").gameObject
	self.SpineCountText=tf:Find("Down/Spine/Text"):GetComponent(typeof(self.Text))
end
...
function PlayerStartView:OnclickStart(isPress)
	...
	if isPress then
		--AudioManager.GetInstance():PlayNormalAudio(47)
		self.AutoStartTimer=TimerManager.GetInstance():CreateTimerInstance(2,self.AutoGame,self)
	else
...
function PlayerStartView:OnclickStop()
	...
	if self.gameData.MainStation==StateManager.MainState.SLB then  Debug.LogError("SLB游戏中不能点击停止")  return end
...
function PlayerStartView:IsShowSpinePanel(isShow)
	CommonHelper.SetActive(self.SpinePanel,isShow)
end

function PlayerStartView:SetSpineCount(count,totalCount)
	self.SpineCountText.text=count.."/"..totalCount
end
