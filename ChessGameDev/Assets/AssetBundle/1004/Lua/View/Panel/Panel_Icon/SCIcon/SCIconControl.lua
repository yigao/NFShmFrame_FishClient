...
function SCIconControl:GetMountPoint(index)
...
local SLBIconNum={5,6,7,8,9,12}
function SCIconControl:ChangeRandomImage()
	local tempIcon=self.IconInsList[1]
	if tempIcon then
		local randomItemNum=0
		self.IsUseFuzzy=false
		if self.gameData.MainStation==StateManager.MainState.Normal  then
			randomItemNum=CommonHelper.GetRandom(0,self.GameConfig.IconItemTotalCount-1)
		elseif self.gameData.MainStation==StateManager.MainState.FreeGame  then
			randomItemNum=CommonHelper.GetRandom(5,self.GameConfig.IconItemTotalCount-1)
		elseif self.gameData.MainStation==StateManager.MainState.SLB  then
			self.IsUseFuzzy=true
			randomItemNum=SLBIconNum[CommonHelper.GetRandom(1,6)]
			
		end
		
		if self.IsUseFuzzy then
			tempIcon:ChangeIconRandomImage(randomItemNum,true)
		else
			tempIcon:ChangeIconRandomImage(randomItemNum)
		end
		if randomItemNum==12 then
			local betChip=BaseFctManager.GetInstance():GetCurrentBetChipValue()
			tempIcon:SetIconText(math.random(betChip,betChip*2))
...
function SCIconControl:SpecifyGameResult(stopIconIndex)
	...
	if tempIconItem then
		...
		self.IconInsList[1]:ChangeIconImage(iconResult,self.IsUseFuzzy)
		self.IconInsList[1]:SetIocnParent(self.MountList[1])
		self.IconInsList[1]:SetParentSiblingIndex(#self.MountList-stopIconIndex)
		self.IconInsList[1]:SetIconValue(iconResult)
		if iconResult==12 then
...
function SCIconControl:SetFirecrackerIconValue(iconIndex)
	...
	IconManager.GetInstance():SetBianPaoIconValue(iconValue,self.IconInsList[1])
...
function SCIconControl:AddDoTween()
	...
	local tempTween=self.gameObject.transform:DOLocalMoveY(self.gameObject.transform.localPosition.y-20, 0.3)
