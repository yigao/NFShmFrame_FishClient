...
function PlayerBetView:OnclickAddBet()
	...
	IconSelectManager.GetInstance():SetIconSelectProcess(self.currentBetIndex,true)	
...
function PlayerBetView:OnclickReduceBet()
	...
	IconSelectManager.GetInstance():SetIconSelectProcess(self.currentBetIndex+1,false)	
	self:SetBetChipValue(self.currentBetIndex)
end


function PlayerBetView:OnclickMaxBet()
	AudioManager.GetInstance():PlayNormalAudio(39)
	self.currentBetIndex=self.totalBetIndex
	self:SetBetChipValue(self.currentBetIndex)
	self:IsEnableButton(self.MaxBetBtn,false)
	self:IsEnableButton(self.AddBetBtn,false)
	self:IsEnableButton(self.ReduceBetBtn,true)
	IconSelectManager.GetInstance():SelectAllIcon()
end
...
function PlayerBetView:InitBetInfo(betInfo)
	...
	if betInfo then
		...
		self.currentBetIndex=betInfo.curChipIndex+1
		self:SetBetChipValue(self.currentBetIndex)
...
function PlayerBetView:SetSingleBetScore(score)
	self.SingleBetScoreText.text="243 x "..CommonHelper.FormatBaseProportionalScore(score)/243--string.format("%.2f",CommonHelper.FormatBaseProportionalScore(score)/243)
end
