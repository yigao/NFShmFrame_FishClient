...
function SCIconControl:InitData()
	...
	self.IconDistance = 0			
	self.ColumnIndex=1				
	self.MoveDirection=CSScript.Vector3.zero		
	self.RunSpeed=3					
...
function SCIconControl:ChangeRandomImage()
	...
	if tempIcon then
		if self.IsUseFuzzy then
			tempIcon:ChangeIconRandomImage(CommonHelper.GetRandom(0,self.GameConfig.IconItemTotalCount-1),true)
		else
			tempIcon:ChangeIconRandomImage(CommonHelper.GetRandom(0,self.GameConfig.IconItemTotalCount-1))
		end
	else
...
function SCIconControl:NormalStopRun(iconResultlist)
	...
	local lastRandomNum=math.random(0,self.GameConfig.IconItemTotalCount-1)
	table.insert(iconResultlist,lastRandomNum)
	--pt(iconResultlist)
...
function SCIconControl:SetGunSpeed()
	
	if	self.IsAccelerate then
		if self.RunSpeed>=5 then
			self.RunSpeed=5
			self.IsAccelerate=false
		else
			self.RunSpeed=self.RunSpeed+CSScript.Time.deltaTime
		end
	elseif self.IsDecelerate then
		if self.RunSpeed<=2 then
			if self.RunSpeed>1.5 then
				if self.IsIconStop==false then
					self.IsIconStop=true
					self.AccelerateRate=math.random(1,2)
				end			
			end
			
			if self.RunSpeed<=1 then
				--self.IsIconStop=true
		...
		self.RunSpeed=self.RunSpeed-CSScript.Time.deltaTime*self.AccelerateRate
	end
