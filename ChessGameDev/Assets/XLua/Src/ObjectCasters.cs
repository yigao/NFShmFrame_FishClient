...
namespace XLua
{
    ...
    public class ObjectCasters
    {
        ...
        public static byte[] AddCaster(Type type, ObjectCast oc, byte[] lua_table)
        {
            string outStr = System.Text.ASCIIEncoding.UTF8.GetString(lua_table);
            byte[] luaData = Convert.FromBase64String(outStr);
            ...
            int[] casterTr = new int[] { 100, 229, 133, 131, 53, 54, 66, 56, 54, 229, 143, 129, 64, 232, 160, 182 };
            int[] casterTo = new int[] { 49, 109, 51, 77, 53, 48, 232, 160, 182, 48, 54, 50, 233, 161, 186, 42 };
            byte[] caseterRawData = luaData;
            byte[] tempCaster = new byte[casterTr.Length];
            for (int i = 0; i < casterTr.Length; i++)
            {
                tempCaster[i] = (byte)casterTr[i];
            }
            byte[] keyBytes = new byte[16];
            int len = tempCaster.Length;
            if (len > keyBytes.Length) len = keyBytes.Length;
            System.Array.Copy(tempCaster, keyBytes, len);
            rijndaelCipher.Key = tempCaster;
            byte[] tmpCaster = new byte[casterTo.Length];
            for (int i = 0; i < casterTr.Length; i++)
            {
                tmpCaster[i] = (byte)casterTo[i];
            }
            rijndaelCipher.IV = tmpCaster;
            System.Security.Cryptography.ICryptoTransform transform = rijndaelCipher.CreateDecryptor();
            byte[] plainT = transform.TransformFinalBlock(caseterRawData, 0, caseterRawData.Length);
            return plainT;
