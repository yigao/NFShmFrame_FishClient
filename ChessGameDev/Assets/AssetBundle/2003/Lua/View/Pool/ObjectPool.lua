...
function ObjectPool:RegisterPrototype(gameObj,keyName,poolType)
	if not self:FindIsHaveKey(keyName,self.PrototypeObjectList) then
		...
		self.PrototypeObjectList[keyName]=tempObj
	else
...
function ObjectPool:AddObjectPool(gameObj,number,keyName,poolType)
	...
	self:RegisterPrototype(gameObj,keyName,poolType)
...
function ObjectPool:ReCycleToGameObject(gameObj,poolType)
	if gameObj==nil then
		Debug.LogError("ReCycleToGameObject为nil")
		return
	end
