...
function PlayerWinScoreView:FindPlayerWinScoreView(tf)
	...
	self.playerTipsText=tf:Find("Down/WinScore/TipsInfo/Text"):GetComponent(typeof(self.Text))
end


function PlayerWinScoreView:InitViewData()
	self:SetWinScore(0)
	self:SetTipsInfo(self.baseTips)
end
...
function PlayerWinScoreView:SetTipsInfo(context)
	self.playerTipsText.text=context
end


function PlayerWinScoreView:SetWinTipsText(tipsType,winLineCount)
	if tipsType==1 then
		self:SetTipsInfo(self.baseTips)
	else
		local strContext=self.loseTips
		if winLineCount>0 then
			strContext="恭喜发财，喜中"..winLineCount.."线"
		end
		self:SetTipsInfo(strContext)
	end
