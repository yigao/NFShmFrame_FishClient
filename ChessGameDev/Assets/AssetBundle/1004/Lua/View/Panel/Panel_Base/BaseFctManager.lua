...
function BaseFctManager:IsShowSpineBtn(isShow)
	self.PlayerStartView:IsShowSpinePanel(isShow)
end

function BaseFctManager:SetSpineCountTips(count,totalCount)
	self.PlayerStartView:SetSpineCount(count,totalCount)
end
...
function BaseFctManager:ClearAllState()
	self:IsEnableBetButton(true)
	self:IsEnableStopButton(true)
	self:IsEnableStartButton(true)
	self:StopAutoGame()
	self.PlayerStartView:InitViewData()
	self.SetView:InitViewData()
	self.AutoView:InitViewData()
	self.PlayerBetView:InitViewData()
...
function BaseFctManager:QuitGame()
	GameManager.GetInstance().isGameing=false
...
function BaseFctManager:SetHandselDataByChangeBet(betIndex)
	if betIndex>#self.gameData.GameBetToChangeOtherDataList then
		Debug.LogError("下注索引超过HandselData长度===>"..betIndex)
		return
	end
	local handselData=self.gameData.GameBetToChangeOtherDataList[betIndex]
	pt(handselData)
	if handselData then
		local handselManager=HandselManager.GetInstance()
		handselManager:SetSingleHandselInsValue(1,handselData.SmallPrize)
		handselManager:SetSingleHandselInsValue(2,handselData.MediumPrize)
		handselManager:SetSingleHandselInsValue(3,handselData.BigPrize)
		handselManager:SetSingleHandselInsValue(4,handselData.MagePrize)
		local iconManager=IconManager.GetInstance()
		iconManager:SetSelectionPrizeValue(1,handselData.ValuePrize2)
		iconManager:SetSelectionPrizeValue(2,handselData.ValuePrize3)
		iconManager:SetSelectionPrizeValue(3,handselData.ValuePrize4)
		iconManager:SetSelectionPrizeValue(4,handselData.ValuePrize5)
		iconManager:SetSelectionPrizeValue(5,handselData.ValuePrize6)
		iconManager:SetSelectionPrizeValue(6,handselData.ValuePrize8)
	end
end
