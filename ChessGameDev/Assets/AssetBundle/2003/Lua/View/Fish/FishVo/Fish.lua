...
function Fish:FindView()
	...
	self.animator=mTransfrom:Find("Bone/Fish"):GetComponent(typeof(self.Animation))
	self.fishBoneTF=mTransfrom:Find("Bone")
end
...
function Fish:FindMainFishRenderView(tf)
	...
	if tempObj then
		self.MainFishSpriteRender=tempObj:GetComponentsInChildren(typeof(self.SkinnedMeshRenderer))
	end
...
function Fish:SetMainFishOrder(orderIndex)
	if self.MainFishSpriteRender then
		for i=0,self.MainFishSpriteRender.Length-1 do
			self.MainFishSpriteRender[i].sortingOrder=orderIndex
		end
	end
end
...
function Fish:IsEnableRenderer(isEnabled)
	if self.MainFishSpriteRender then
		for i=0,self.MainFishSpriteRender.Length-1 do
			self.MainFishSpriteRender[i].enabled=isEnabled
		end
	end
end
...
function Fish:SetMainFishColor(color)
	if self.MainFishSpriteRender then
		for i=0,self.MainFishSpriteRender.Length-1 do
			self.MainFishSpriteRender[i].sharedMaterial.color=color
		end
	end
end
