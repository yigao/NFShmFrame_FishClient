	...
	local StopRunMidIconFunc=function ()
		if self.RunIconItemInsList and #self.RunIconItemInsList>0 then 
			for i=1,4 do
				if self.RunIconItemInsList[i] then
					local midRt={}
					local result=self.MidIconResult[i]-1
					if result then
...
function BonusManager:CheckTimer()
	local PlayBonusFunc=function ()
		self:RecycleTimer()
		self:IsContinuePlayBonus()
		--Debug.LogError("断线重连发送消息中...")
	end
	self.PlayBonusTimer=TimerManager.GetInstance():CreateTimerInstance(5,PlayBonusFunc)
end


function BonusManager:RecycleTimer()
	if self.PlayBonusTimer then
		TimerManager.GetInstance():RecycleTimerIns(self.PlayBonusTimer)
		self.PlayBonusTimer=nil
	end
end
