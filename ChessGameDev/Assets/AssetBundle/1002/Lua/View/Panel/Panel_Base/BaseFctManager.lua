...
function BaseFctManager:AddScripts()
	self.ScriptsPathList={
		...
		"View/Panel/Panel_Base/Bonus/BonusView",
	}
end
...
function BaseFctManager:ClearWinScore()
	self.PlayerWinScoreView:SetWinScore(0)
	self.PlayerWinScoreView:SetAddScore(0)
end


function BaseFctManager:InitBetInfo(betInfo)
	return self.PlayerBetView:InitBetInfo(betInfo)
end
...
function BaseFctManager:IsEnableRecieveButton(isEnable)
	self.PlayerStartView:IsEnableRecieveButton(isEnable)
end
...
function BaseFctManager:IsShowStartBtnEffect(isShow)
	self.PlayerStartView:IsShowStartEffect(isShow)
end


function BaseFctManager:IsShowRecieveBtnEffect(isShow)
	self.PlayerStartView:IsShowRecieveEffect(isShow)
end

function BaseFctManager:IsShowPlayerWinScoreEffect(isShow)
	self.PlayerWinScoreView:IsShowPlayerWinScoreEffect(isShow)
end

function BaseFctManager:IsShowButtonEffect(isShow)
	self:IsShowStartBtnEffect(isShow)
	self:IsShowRecieveBtnEffect(isShow)
	self:IsShowPlayerWinScoreEffect(isShow)
end
...
function BaseFctManager:SetBonusProcess(callbackFunc)
	self.BonusView:SetBonusProcess(callbackFunc)
end


function BaseFctManager:SetJackpotProcess(score,callbackFunc)
	self.BonusView:SetJackpotProcess(score,callbackFunc)
end



function BaseFctManager:RecieveScoreProcess(data)
	self:IsEnableRecieveButton(false)
	self:IsShowRecieveBtnEffect(false)
	self.gameData.AddPlayerWinScore=data.totalWinScore
	self.gameData.PlayerMoney=data.userScore
	self.PlayerWinScoreView:RecieveScoreProcess()
end
...
function BaseFctManager:ClearAllState()
	self:SetPlayerWinScore(0,0,false)
