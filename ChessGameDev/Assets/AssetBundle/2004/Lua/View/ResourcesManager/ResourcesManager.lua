...
function ResourcesManager:CalculateTotalResourcesCount()
	...
	totalCount = totalCount + #gameConfig.SpecialDeclareRes

	totalCount=totalCount+#gameConfig.LightningRes

	totalCount=totalCount+#gameConfig.EffectRes

	totalCount = totalCount + #gameConfig.SkillRes
...
function ResourcesManager:CorotineLoadResources()	
	...
	table.insert(self.queue,function() self:CreateSkillPool() end)
	table.insert(self.queue,function() self:CreateFishOutTipsPool() end)
	table.insert(self.queue,function() self:CreateSpecialDeclarePool() end)
...
function ResourcesManager:InitGameConfig()
	...
	self.gameData.gameConfigList.SpecialDeclareConfigList=self:AlignGameConfig(self.gameData.gameConfigList.SpecialDeclareConfigList)
...
function ResourcesManager:CreateBulletPool()
	...
	if totalCount>0 then
		for k,v in pairs(self.gameData.GameConfig.BulletRes) do
			...
			local parentPrefab = self.FishPackResList[v.ParentName]
			local prefab = parentPrefab.transform:Find(v.name).gameObject
			GameObjectPoolManager.GetInstance():AddGameObjectPool(prefab,v.amout,v.name,GameObjectPoolManager.PoolType.BulletPool)
			self:LoadProgressBarEvent()
		end
	end
...
function ResourcesManager:CreateSkillPool()
	local totalCount=#self.gameData.GameConfig.SkillRes
	local count = totalCount
	if totalCount>0 then
		for k,v in pairs(self.gameData.GameConfig.SkillRes) do	
			local CreateSkillPoolCallBackFunc=function (gameObj)
				if gameObj and gameObj.content then
					local tempSkill=CommonHelper.Instantiate(gameObj.content)
					GameObjectPoolManager.GetInstance():AddGameObjectPool(tempSkill,v.amout,v.name,GameObjectPoolManager.PoolType.EffectPool)
					CommonHelper.Destroy(tempSkill)
					self:LoadProgressBarEvent()
					count=count-1
					if count<=0 then
						self:ExcuteLoadResourcesQueue()
			...
			yield_return(0)
			GameManager.GetInstance():AsyncLoadResource(v.path,typeof(CSScript.GameObject),CreateSkillPoolCallBackFunc)
		end
	end
end
...
function ResourcesManager:CreateSpecialDeclarePool()
	local totalCount=#self.gameData.GameConfig.SpecialDeclareRes
	local count = totalCount
	if totalCount>0 then
		for k,v in pairs(self.gameData.GameConfig.SpecialDeclareRes) do		
			local CreateSpecialDeclarePoolCallBackFunc=function (gameObj)
				if gameObj and gameObj.content then
					local tempBigAwardTips=CommonHelper.Instantiate(gameObj.content)
					GameObjectPoolManager.GetInstance():AddGameObjectPool(tempBigAwardTips,v.amout,v.name,GameObjectPoolManager.PoolType.SpecialDeclarePool)
			...
			yield_return(0)
			GameManager.GetInstance():AsyncLoadResource(v.path,typeof(CSScript.GameObject),CreateSpecialDeclarePoolCallBackFunc)
		end
	end
end
