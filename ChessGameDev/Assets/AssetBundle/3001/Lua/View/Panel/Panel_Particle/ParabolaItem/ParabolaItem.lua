...
function ParabolaItem:Update()
	if self.isFly then
		...
		if self.currentTime>self.totalTime and self.currentTime<self.hideTime then
			...
			if self.HeadEffect==nil then
				self.HeadEffect=ParticleManager.GetInstance():GetAllocateEffect("PlayerHeadEffect")
				self.HeadEffect.transform.position=self.endPos
				local DelayRecycleHeadEffectFunc=function ()
					if headEffectTimer then
						TimerManager.GetInstance():RecycleTimerIns(headEffectTimer)
					end
					if self.HeadEffect then
						ParticleManager.GetInstance():RecycleEffect(self.HeadEffect)
					end
				end
				local headEffectTimer=TimerManager.GetInstance():CreateTimerInstance(2,DelayRecycleHeadEffectFunc)
			end
