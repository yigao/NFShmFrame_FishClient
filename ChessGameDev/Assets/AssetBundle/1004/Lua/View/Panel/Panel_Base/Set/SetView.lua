...
function SetView:FindSetView(tf)
	...
	self.HelpBtn=tf:Find("Up/SetPanel/HelpBtn"):GetComponent(typeof(self.Button))
end
...
function SetView:OnclickQuitGame()
	...
	if GameLogicManager.GetInstance():GetCurrentExitState()==false then
		--GameManager.GetInstance():ShowUITips("抽奖过程 或者 小游戏 状态不能退出",3)
		HandselManager.GetInstance():ShowTips("抽奖过程 或者 小游戏 状态不能退出")
		return
	end
