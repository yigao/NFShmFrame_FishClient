...
function ResourcesManager:InitData()
	...
	self.Texture2D = GameManager.GetInstance().Texture2D
...
function ResourcesManager:CalculateTotalResourcesCount()
	...
	totalCount = totalCount + #gameConfig.OtherTexRes
...
function ResourcesManager:CorotineLoadResources()	
	...
	table.insert(self.queue,function() self:InitOtherTexture() end)
...
function ResourcesManager:InitOtherTexture()
	local totalCount=#self.gameData.GameConfig.OtherTexRes
	local count = 0
	for k,v in pairs(self.gameData.GameConfig.OtherTexRes) do
		local CreateOtherTexCallBack=function (gameObj)
			if gameObj and gameObj.content then
				self:LoadProgressBarEvent()
				count=count + 1
		...
		yield_return(0)
		GameManager.GetInstance():AsyncLoadResource(v.path,typeof(self.Texture2D),CreateOtherTexCallBack)	
	end
end
