...
function GameManager:GameAssetLoadCompleteCallBack()
	...
	GlobalEventManager.GetInstance():DispatchEvent(LuaEventParams.LoadGameAssetsComplete_EventName,false,self.gameData.gameConfigList.RoomConfig_1001List)
end
...
function GameManager:InitGameState(gameType)
	...
	if gameType>0 then
		...
		elseif gameType==StateManager.MainState.Bonus then
			GameLogicManager.GetInstance():IsBonusProcess()
