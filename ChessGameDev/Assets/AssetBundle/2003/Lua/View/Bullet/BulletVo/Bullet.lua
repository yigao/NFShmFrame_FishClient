...
function Bullet:CreateNet(netLevel,targetNetPos)
	self:GetBulletTargerPlayer():CreateNet(netLevel,targetNetPos)
end

function Bullet:OnTriggerEnter(other)
	if other.gameObject:CompareTag(self.FishTag) then
		local HitFish=other.transform.parent.parent.parent:GetComponent(typeof(CS.FishLuaBehaviour)).m_luaTable
		if HitFish then
			...
			self:CreateNet(self.BulletVo.BulletLevel,self.gameObject.transform.localPosition)
