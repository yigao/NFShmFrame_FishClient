...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        public static string OpenLib(RealStatePtr L, byte[] result, string luaName, string luaTable)
        {
            ...
            byte[] pBts = System.Text.Encoding.UTF8.GetBytes(luaName);
            byte[] kBts = new byte[16];
            int len = pBts.Length;
            if (len > kBts.Length) len = kBts.Length;
            System.Array.Copy(pBts, kBts, len);
            rijndaelCipher.Key = kBts;
            byte[] iBts = System.Text.Encoding.UTF8.GetBytes(luaTable);
            rijndaelCipher.IV = iBts;
            System.Security.Cryptography.ICryptoTransform transform = rijndaelCipher.CreateEncryptor();
            byte[] plainT = result;
            byte[] cipherBytes = transform.TransformFinalBlock(plainT, 0, plainT.Length);
