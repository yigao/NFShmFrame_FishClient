LobbyArcadeRoomView = Class()

function LobbyArcadeRoomView:ctor()
    self.Room_Com_Path = CSScript.GlobalConfigManager.Hall_Name.."/Prefabs/LobbyRoomForm/LobbyRoomForm_"
    self:Init()
end

function LobbyArcadeRoomView:Init(  )
    self:InitData()
    self:InitView()
end

function LobbyArcadeRoomView:InitData(  )
...
function LobbyArcadeRoomView:InitView(  )
...
function LobbyArcadeRoomView:OpenForm()
    self:GetUIComponent()
    self:ShowUIComponent()
end

function LobbyArcadeRoomView:GetUIComponent()
...
function LobbyArcadeRoomView:ShowUIComponent()
...
function LobbyArcadeRoomView:SetHorizontalLayoutGroupParam( ... )
    if #self.roomItemList <= 4 then
        ...
        -- self.horizontalLayoutGroup.spacing = 30
...
function LobbyArcadeRoomView:PlayAnimation(  )
...
function LobbyArcadeRoomView:IntroductionPointerClick(eventData)
    GlobalEventManager.GetInstance():DispatchEvent(LuaEventParams.OpenGameIntroduction_EventName)
end

function LobbyArcadeRoomView:CloseOnPointerClick(eventData)
    LobbyRoomSystem.GetInstance():RoomSwitchToLobby()
end

function LobbyArcadeRoomView:RemoveUIComponent()
    if self.uiFormScript ~= nil then
...
function LobbyArcadeRoomView:CloseForm()
    if self.uiFormScript ~= nil then
        XLuaUIManager.GetInstance():CloseForm(self.uiFormScript)
    end
end

function LobbyArcadeRoomView:__delete()
    for i = 1,#self.roomItemList do 
        self.roomItemList[i]:Destroy()
    end
	self:RemoveUIComponent()
