...
function IconManager:InitData()
	...
	self.BonusIconImagePrefixName="Item"	--Bonus图标前缀
end
...
function IconManager:GetBonusIconItemByIconNum(iconNum)
	local tempIconItem=self:GetIconItembyIconNum(iconNum,self.BonusIconImagePrefixName,GameObjectPoolManager.PoolType.IconPool)
	if tempIconItem then
		return tempIconItem
	else
...
function IconManager:IsChangAllIconItemColor(isChange)
	for i=1,#self.IconCInsList do
		local tempCInsList=self.IconCInsList[i]:GetIconInsList()
		for j=1,#tempCInsList do
			tempCInsList[j]:IsChangeMainImageColor(isChange)
		end
