...
function ObjectPool:RecycleAllocateTypeGameObject(gameObjName,poolType)
	if self:FindIsHaveKey(gameObjName,self.ObjectPoolList) then
		for i=1,#self.ObjectPoolList[gameObjName] do
			if self.ObjectPoolList[gameObjName][i].activeSelf then
				self:ReCycleToGameObject(self.ObjectPoolList[gameObjName][i],poolType)
			end
		end
